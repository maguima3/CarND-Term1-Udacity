...
The dataset contains traffic signals of the German Traffic Signs dataset. As mentioned before, the traffic signs are categorized into 43 different classes, according to what they represent. Here are some examples of the images:
...
My model architecture was first based on LeNet: 2 convolutional layers + 2 fully connected layer + 1 output layer. After training the model with different combinations of hyperparameters, the validation accuracy was never higher than 87%. Inspired by Sermanet/Yann LeCun paper, I decided to add one more convolutional layer, and the performance of the network notably increased.
...
I believe that the model did not correctly classify the “End of all speed and passing limits” sign because the original number of samples provided in the original data set was low (only 210 samples). Even though the training set was extended to 2.500 samples with rotated and shifted transformations of the original images, it seems not have been sufficient. I am confident that improving the data augmentation process  will lead to better results in the future.
...
The images above show the feature maps of the first and second convolutional layers of the model, with 16 and 32 filters.

As expected, the first convolutional layer detects very basic pixel patterns, like edges and lines. These basic filters are then used by subsequent layers as building bricks to construct detectors of more complicated patterns and figures.
