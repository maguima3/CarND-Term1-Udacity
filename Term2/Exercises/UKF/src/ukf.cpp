...
void UKF::PredictMeanAndCovariance(VectorXd* x_out, MatrixXd* P_out) {
	...
	//matix with predicted sigma points
	MatrixXd Xsig_pred = MatrixXd(n_x, 2*n_aug+1);
	Xsig_pred <<
...
for (int i=0; i<2*n_aug+1; i++) {
	...
	//transformation to measurement model
...
z_pred.fill(0.0);
for (int i=0; i<2*n_aug+1; i++) {
	// z_pred(0, i) = z_pred(0, i) + weights(i) * Zsig(0, i);
	// z_pred(1, i) = z_pred(1, i) + weights(i) * Zsig(1, i);
	// z_pred(2, i) = z_pred(2, i) + weights(i) * Zsig(2, i);
...
while (z_diff(1)>M_PI) z_diff(1)-=2.*M_PI; //angle norm
while (z_diff(1)<-M_PI) z_diff(1)+=2.*M_PI;
