...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    // TODO: Deal with special initialization cases:
    // E.g: what happens if the first measurement is zero??
    ...
    //save initial timestamp for dt calculation
  ...
  //state transition matrix F update according to the new elapsed time: time is integrated
  ...
  //acceleration noise components from task
  float noise_ax = 9.0;
  float noise_ay = 9.0;
  Q_noise << noise_ax, 0,
             0, noise_ay;

  //precompute useful values to speed up calculation of Q
