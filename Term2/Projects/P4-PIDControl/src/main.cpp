...
int main()
{
  ...
  PID pid_s(5, 1);
  pid_s.Init(0.07, 0.003, 0.8);

  //debug
  std::cout << "---- Steering PID Controller ----" << std::endl;
  std::cout << "Kp=" << pid_s.Kp << "\tKi=" << pid_s.Ki << "\tKd=" << pid_s.Kd <<
               "\tNi=" << pid_s.N_i << "\tNd=" << pid_s.N_d << std::endl;
  //throttle PID controller
  PID pid_t(0, 1);
  pid_t.Init(0.1, 0.0, 0.4);
  std::cout << "---- Throttle PID Controller ----" << std::endl;
  std::cout << "Kp=" << pid_t.Kp << "\tKi=" << pid_t.Ki << "\tKd=" << pid_t.Kd <<
               "\tNi=" << pid_t.N_i << "\tNd=" << pid_t.N_d << std::endl;
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          throttle_value = max_throttle - (pid_t.Kp * pid_t.p_error +
